...
struct Drive {
    ...
    int init_mode;
    int control_mode;
    /* When set (human PBT train): non-ego active agents follow WOMD traj via
     * move_expert (same buffer packing as replay; ego_ratio selects learners). */
...
void c_step(Drive *env) {
    ...
    for (int i = 0; i < env->active_agent_count; i++) {
        ...
        /* Human: non-ego buffer slots are log-replay (WOMD traj), like replay's
         * population partners — still in the obs/action buffer, not experts outside.
         * When num_ego_local==0 (map got no egos from global sampling), every local
         * is a partner and must still move_expert — do not require num_ego_local>0. */
        if (env->partners_as_experts && !is_ego_local(env, i)) {
            move_expert(env, env->actions, agent_idx);
        } else {
